#This code script was used to process upper limb accelerometry data from the study, Upper Limb Activity in Adults:
#Referent Values Using Accelerometry (PMC3905245) for the project:
...
#and dominant side (column 3). Note that this script processes
...
colnames(DATAPROCESSINGLOG)[3] <- "DominantHand"
...
for(s in 1:nrow(DATAPROCESSINGLOG)) {
    ...
    #Extract dominant side
    DomSide <- DATAPROCESSINGLOG$DominantHand[s]
    
    # Inside of each time point
    L_dir <- list.files(sub_dir, pattern = "LUE")
    L_dir <- paste(sub_dir, L_dir, "/", sep = "")

    R_dir <- list.files(sub_dir, pattern = "RUE")
    R_dir <- paste(sub_dir, R_dir, "/", sep = "")
    ...
    #Remove the first 2 hours of the recording due to in-lab time
    if(LENG_1 >86400){
      InLabTime <- LENG_1 - 86400
      Combined1Hz <- Combined1Hz[-c(1:InLabTime), ]
    }
    ...
    #Remove the first part of the recording for in-lab time
    if(LENG_30 > 2592000){
      InLabTime <- LENG_30 - 2592000 
      L_30_DONE_FILE <- L_30_DONE_FILE[-c(1:InLabTime), ]
      R_30_DONE_FILE <- R_30_DONE_FILE[-c(1:InLabTime), ]
    }
    ...
    #Separate into days (only one recording day, per study protocol)
    ...
    #####SAVE TO FILES#####
    STUDYPROTOCOL <- "AdultReferent"
    #Save to source data participant's folder
    WriteCSVFilename <- paste0(sub_dir, sub_id, "_", STUDYPROTOCOL, "_SleepIncOutputReprocessed.csv", sep = "") 
    #Save to harmonized data folder
