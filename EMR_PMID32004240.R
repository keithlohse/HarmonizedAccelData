#This code script was used to process upper limb accelerometry data for the study, Activity Level and Intensity of Older Adults
#in Skilled Nursing Rehabilitation Measured via Actigraphy (PMID32004240) for the project:
#Harmonized Upper and Lower Limb Accelerometry Data
#The code reads in a data processing log that contains the participant's ID number (column 1), affected side (column 5),
#and hand dominance (column 6). Note that this script processes
#one day of accelerometry data, per the study protocol. Participants in this cohort were residing in a skilled nursing facility.
...
colnames(DATAPROCESSINGLOG)[2] <- "Diagnosis"
colnames(DATAPROCESSINGLOG)[3] <- "DiagnosisCode"
colnames(DATAPROCESSINGLOG)[4] <- "MoreThanOneDiagnosis"
colnames(DATAPROCESSINGLOG)[5] <- "AffectedSide"
colnames(DATAPROCESSINGLOG)[6] <- "Dominance"
colnames(DATAPROCESSINGLOG)[7] <- "AccelDate"

#Replace . with NA in AffectedSide column
DATAPROCESSINGLOG$AffectedSide <- ifelse(DATAPROCESSINGLOG$AffectedSide == ".", "NA",
                                             DATAPROCESSINGLOG$AffectedSide)

#Reformat the AccelDate column to match folder convention
DATAPROCESSINGLOG$AccelDateFolder <- strptime(DATAPROCESSINGLOG$AccelDate, "%m/%d/%Y")
DATAPROCESSINGLOG$AccelDateFolder <- format(DATAPROCESSINGLOG$AccelDateFolder, "%Y%m%d")
DATAPROCESSINGLOG$FolderID <- paste(DATAPROCESSINGLOG$StudyID, "_PTOT_", DATAPROCESSINGLOG$AccelDateFolder, sep = "")

#Recode AffectedSide and Dominance columns and create new NonDominant column
DATAPROCESSINGLOG$AffectedSide <- revalue(DATAPROCESSINGLOG$AffectedSide, c("0" = "Right", "1" = "Left", "2" = "NA", "NA" = "NA"))
DATAPROCESSINGLOG$Dominance <- revalue(as.character(DATAPROCESSINGLOG$Dominance), c("0" = "Right", "1" = "Left"))
DATAPROCESSINGLOG$NonDominant <- revalue(DATAPROCESSINGLOG$Dominance, c("Right" = "Left", "Left" = "Right"))
...
for(s in 1:nrow(DATAPROCESSINGLOG)) {
    ...
    folder_name <- DATAPROCESSINGLOG$AccelDateFolder[s]
    sub_dir <- paste(main_dir, "/",sub_id,"/", sep="")

    #Identify correct subfolder/accel data collection for each participant
    FolderLocation <- list.files(sub_dir, pattern = folder_name)
    FolderLocationPath <- paste(sub_dir, FolderLocation, sep = "")
    print(FolderLocationPath)

    #Extract affected side: for EMR study, use AffectedSide data, but if there is no AffectedSide, use Dominance data
    AffSide <- ifelse(DATAPROCESSINGLOG$AffectedSide[s] == "NA", DATAPROCESSINGLOG$NonDominant[s], DATAPROCESSINGLOG$AffectedSide[s])

    #Inside of each time point
    L_dir <- list.files(FolderLocationPath, pattern = "LUE")
    L_dir <- paste(FolderLocationPath, "/", L_dir, "/", sep = "")
    print(L_dir)

    R_dir <- list.files(FolderLocationPath, pattern = "RUE")
    R_dir <- paste(FolderLocationPath, "/", R_dir, "/", sep = "")
    print(R_dir)
    ...
    if (AffSide == "Right") {
      #Use left to Find length of dataset
    ...
    #Compute vector magnitudes for left and right sides
    ...
    #Separate into days (only one recording day for this cohort)
    ...
    WriteCSVFilename <- paste0(FolderLocationPath, "/", sub_id, "_", folder_name, "_", STUDYPROTOCOL, "_SleepIncOutputReprocessed.csv", sep = "") 
    ...
    TaskData <- data.frame(StudyID = sub_id, AccelDate = folder_name, TaskData)
