#This code script provides an overview of the upper limb accelerometry processing procedures for the project:
...
#time point of data collection (column 2), and affected side (column 3). Note that this script processes
#one day of accelerometry data but could be modified to process multiple days of accelerometry data.
...
colnames(DATAPROCESSINGLOG)[3] <- "AffectedSide"

#Remove spaces from Event.Name for matching to TimePoint later on
DATAPROCESSINGLOG$TimePoint <- str_replace_all(DATAPROCESSINGLOG$TimePoint, " ", "")
...
for(s in 1:nrow(DATAPROCESSINGLOG)) {
    ...
    #LENG.LAST.Day1 is the length of the data set up to the last whole hour.
      for (X in 1:LENG.LAST.Day1){
        
        #HOUR.SAMPLINGRATE = the frames in one hour based on the capture hertz
    ...
    #Separate into days (only one recording day in this script, but could me modified if >1 day)
    ...
    #Calculating frequency variables
    ...
    #####SAVE TO FILES#####
    STUDYPROTOCOL <- "Study Protocol Name"
